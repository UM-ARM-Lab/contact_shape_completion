...
def fit_2_5D_view(metadata, reference):
    sn = shape_completion_training.utils.old_dataset_tools.get_addressible_dataset(use_train=False, dataset_name=DATASET)
    print("Loading plausibilities")

    valid_fits = plausiblility.get_plausibilities_for(
        shape_completion_training.utils.old_dataset_tools.get_unique_name(reference), dataset_name=DATASET)
    ...
    for elem_name, T, p, oob in valid_fits:
        ...
        p = shape_completion_training.model.observation_model.observation_likelihood_geometric_mean(reference['gt_occ'],
                                                                                                    fitted,
                                                                                                    std_dev_in_voxels=2)
        print("Best fit for {}: p={}".format(shape_completion_training.utils.old_dataset_tools.get_unique_name(elem), p))
        rospy.sleep(0.1)
...
if __name__ == "__main__":
    ...
    train_records, test_records = shape_completion_training.utils.old_dataset_tools.load_dataset(dataset_name=DATASET, metadata_only=True,
                                                                                                 shuffle=False)
